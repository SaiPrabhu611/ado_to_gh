  ...
  foreach ($line in $lines[1..($lines.Count-1)]) {
    ...
    if (-not $seen_orgs.ContainsKey($s_org)) {
      ...
      if ($orgVarsJson) {
        ...
        $vars = @()
        if ($orgVarsObj.PSObject.Properties.Name -contains "variables") { $vars = @($orgVarsObj.variables) } else { $vars=@() }
    ...
    if ($repoVarsJson) {
      ...
      $vars = if ($repoVarsObj.PSObject.Properties.Name -contains "variables") { @($repoVarsObj.variables) } else { @() }
    ...
    if ($envsJson) {
      ...
      $envNames = @()
      if ($envsObj.PSObject.Properties.Name -contains "environments") { $envNames = @($envsObj.environments | ForEach-Object { $_.name }) }
